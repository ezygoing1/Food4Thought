...
let FoodTruck = require('../app/models/foodTruck,js')
...
describe('Examples', () => {
  ...
  before(done => {
    Example.deleteMany({})
      ...
      .then(() => Example.create(Object.assign(exampleParams, {owner: foodTruckId})))
      .then(record => {
  ...
  describe('DELETE /examples/:id', () => {
    ...
    before(done => {
      Example.create(Object.assign(exampleParams, { owner: foodTruckId }))
        .then(record => {
          exampleId = record._id
          done()
        })
    ...
    it('must be owned by the foodTruck', done => {
      chai.request(server)
  ...
  describe('POST /examples', () => {
    ...
    it('should not allow a POST from an unauthenticated foodTruck', done => {
      chai.request(server)
        .post('/examples')
  ...
  describe('PATCH /examples/:id', () => {
    ...
    before(async function () {
      const record = await Example.create(Object.assign(exampleParams, { owner: foodTruckId }))
      exampleId = record._id
    })
